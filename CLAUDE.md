...
The Functional suite needs MariaDB/MySQL and Meilisearch with master key `aSampleMasterKey`. Start Meilisearch with `cd tests/Application && docker compose up -d --wait` (or `tests/Application/meilisearch.sh` without Docker; the CI service container is in `.github/workflows/build.yaml`). Keep the compose image version in sync with the CI service containers. The suite also needs the test app frontend built once (`yarn install && yarn build`, see "Test app frontend") — tests like `TaxonPageTest` render full shop pages, and a missing Encore build fails loudly (on the lowest symfony/asset 6.4.0 even a missing `manifest.json` throws regardless of `strict_mode`).
...
`setono_sylius_meilisearch.indexes` config becomes `Config\Index` value objects (name, Document class, entity classes, per-index service locator, `dynamic_fields` flag) collected in `Config\IndexRegistry`. Indexing is triggered by the `setono:sylius-meilisearch:index` command or by Doctrine lifecycle events (`EventListener\Doctrine\EntityListener`), both dispatching messenger commands (`Message\Command\*`). `Indexer\DefaultIndexer` batches entity ids through `IndexBuffer`, loads entities, and runs each through the composite `DataMapper` chain, which populates `Document\Document` subclasses (attribute mapping is driven by PHP attributes like `#[Facetable]` on document properties, read via `Document\Metadata\MetadataFactory`). Documents are serialized and pushed to Meilisearch; the index uid is resolved per scope (channel/locale/currency + `%env(MEILISEARCH_PREFIX)%` and kernel environment) via `Resolver\IndexUid` and `Provider\IndexScope`.

Metadata is a function of **(document class, index)**: `MetadataFactoryInterface::getMetadataFor($document, $index)` requires the index, memoizes per `class|index`, and dispatches `Event\MetadataCreated` **before** memoizing — so a listener must never call `getMetadataFor()` (infinite recursion); it mutates `$event->metadata` and reads repositories instead. `MetadataFactory` implements `ResetInterface` with an explicit `kernel.reset` tag (the plugin's XML has no autoconfiguration, so the tag must be explicit) — messenger's per-message reset is how workers see admin configuration changes.

### Dynamic fields (admin-configurable indexed attributes/options)

The `IndexableAttribute`/`IndexableOption` resources (Synonym-style admin stack, shared `IndexableSubject` base + `IndexableSubjectRepository`) reference their Sylius product attribute/option via a many-to-one with a **unique** join column and `on-delete: CASCADE`. `EventSubscriber\IndexableSubjectMetadataSubscriber` merges enabled rows into the metadata of eligible indexes as flat `attr_<code>`/`opt_<code>` fields. Invariants to preserve:

- **Facetable must imply filterable**: `DefaultSettingsProvider` only pushes `getFilterableAttributeNames()`, and `MultiSearchBuilder` requests every facetable name via `setFacets()` on every search — a facetable name missing from Meilisearch's `filterableAttributes` fails the whole request. (`SearchEngine` retries once without facets on `invalid_search_facets` to cover the settings-task window.)
- **Eligibility** is `Config\Index::supportsDynamicFields()` (the `dynamic_fields` flag AND at least one entity implementing `ProductInterface`/`ProductVariantInterface`), enforced in the `IndexChoiceType` form option `dynamic_fields_only`, the grid filters, and the subscriber (stale rows stay inert).
- **Facet type coercion** (`Facet->type` drives filter/form builders): select/text → `array`, checkbox → `bool`, integer → `int`, float → `float`, date/datetime → `string` = searchable/filterable but **never** a facet.
- Values flow through `DataMapper\Product\DynamicFieldsDataMapper` (reuses the localized values providers; a **variant** source gets its parent product's attributes and its own option values) into the `Document::$dynamicFields` bag, which `Normalizer\DocumentNormalizer` flattens to top-level fields (declared document fields always win).
- Saving rows triggers `EventListener\Doctrine\IndexableSubjectListener`: it collects affected index names (including indexes *removed* from a row, via the `preUpdate` changeset), resets the metadata memo, and dispatches one `Index` command per index on kernel response.
