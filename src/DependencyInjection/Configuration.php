...
INFO, ToggleableInterface::class, QueryBuilderForDataProvisionCreated::class, EntityFilterInterface::class),
                ...
                ->arrayNode('server')
                    ...
                    ->children()
                        ->scalarNode('url')
                            ->info('This is the URL of the Meilisearch instance')
                            ->defaultValue('%env(MEILISEARCH_URL)%')
                            ->cannotBeEmpty()
                        ->end()
                        ->scalarNode('public_url')
                            ->info('The browser-facing URL of the Meilisearch instance. This is used by the autocomplete widget, which talks to Meilisearch directly from the browser. Set this (e.g. to %env(MEILISEARCH_PUBLIC_URL)%) when the server-side URL is an internal hostname that is unreachable from browsers, as is common in containerized setups. When null or empty it falls back to the server URL.')
                            ->defaultNull()
                        ->end()
                ...
                ->arrayNode('search')
                    ...
                    ->children()
                        ...
                        ->arrayNode('taxon')
                            ->canBeDisabled()
                            ->info('When enabled (the default), the search page takes over Sylius\' sylius_shop_product_index route so taxon pages are served by Meilisearch. Disable to keep Sylius\' stock taxon pages')
                        ->end()
                    ->end()
                ->end()
